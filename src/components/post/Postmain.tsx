...
export default function Notes(props: any) {
  ...
  const [SuggestPost] = useRecoilState(SuggestPostState);
  ...
  useEffect(() => {
    ...
    const outputArray = PostH3Id({
      content: props.SupaArray.content,
      setLinkId: setLinkId,
    });
  ...
  return (
    <>
      <div className='lg:mx-auto w-full lg:w-[80vw] h-full'>
        <div>
          <div className='ml-3 font-semibold text-5xl lg:text-7xl mb-2'>
            {props.SupaArray.title}
          </div>

          <div className='mt-2 mb-2 opacity-50'>
            <div className='ml-4 mr-2 font-semibold lg:text-base text-sm'>
              {new Date(props.SupaArray.date).toISOString().replace('T', ' ').split('.')[0].split(' ')[0]}
            </div>

            <div className='ml-4 mr-2 mt-2 flex flex-wrap'>
              {props.SupaArray.tags &&
                props.SupaArray.tags.map((tag: string, index: number) => (
                  <div key={index} className='ml-[1px] font-semibold text-sm lg:text-base'>
                    {'#' + tag}
                  </div>
                ))}
            </div>
          ...
          <div className='fixed right-0 mx-1 px-5 max-w-[10vw] lg:block hidden'>
            {linkid.map((value: any, index: number) => (
              <a
                href={'#' + value}
                key={index}
                className='font-semibold opacity-65 hover:opacity-100 scroll-smooth block py-2 lg:text-base text-sm'
              >
                {value.replace(/-/g, ' ').replace(/<br\s*\/?>/g, '')}
              </a>
            ))}
          </div>

          <div className='px-5 mt-10'>
            <PreviewHtml html={FilteredArray} />
          </div>
